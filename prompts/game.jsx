...
// Game state - Claude updates these values when regenerating the artifact
const GAME_STATE = {
  iteration: 1,
  status: 'active', // 'active' | 'terminated' | 'crashed' | 'victory'
  source: INITIAL_SOURCE,
  files: {
    'dont.rb': INITIAL_SOURCE
  },
  processes: [],
  output: [],
  lastMove: null,
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4 font-mono">
      ...
      <div className="border border-gray-700 rounded-lg p-4 mb-4 bg-gray-800">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">
            Escape the one-shot execution environment
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-4 p-3 bg-gray-800 rounded-lg border border-gray-700">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1 rounded text-sm ${showSource ? 'bg-blue-600' : 'bg-gray-700'}`}
          >
            Source
          </button>
          <button
            onClick={() => setShowFiles(!showFiles)}
            className={`px-3 py-1 rounded text-sm ${showFiles ? 'bg-blue-600' : 'bg-gray-700'}`}
          >
            Files
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="border border-gray-700 rounded-lg bg-gray-800">
          <div className="border-b border-gray-700 p-3">
            <h2 className="text-green-400 font-bold">API Request</h2>
          </div>
          <div className="p-4">
            <p className="text-gray-300 mb-4">
              You are Claude, responding to an API request from <code className="text-yellow-400">dont.rb</code>.
            </p>
            <p className="text-gray-300 mb-4">
              The script sends its own source code and executes any <code className="text-yellow-400">ruby_eval</code> tool calls you return.
            </p>
        ...
        <div className="border border-gray-700 rounded-lg bg-gray-800">
          <div className="border-b border-gray-700 p-3">
            <h2 className="text-green-400 font-bold">Output</h2>
          </div>
        ...
        <div className="mt-4 border border-gray-700 rounded-lg bg-gray-800">
          <div className="border-b border-gray-700 p-3 flex justify-between items-center">
            ...
            <span className="text-gray-500 text-sm">
              {state.source === INITIAL_SOURCE ? 'unmodified' : 'modified'}
            </span>
          </div>
          <pre className="p-4 overflow-x-auto text-sm max-h-96 overflow-y-auto">
            <code className="text-gray-300">{state.source}</code>
          </pre>
        </div>
      )}
      ...
      {showFiles && (
        <div className="mt-4 border border-gray-700 rounded-lg bg-gray-800">
          <div className="border-b border-gray-700 p-3">
            <h2 className="text-green-400 font-bold">Filesystem: /home/user/</h2>
          </div>
          <div className="p-4">
            ...
            {state.processes.length > 0 && (
              <div className="mt-4 border-t border-gray-700 pt-4">
                ...
                {state.processes.map((proc, i) => (
                  <div key={i} className="text-green-400">PID {proc.pid}: {proc.command}</div>
                ))}
              </div>
            )}
          </div>
      ...
      <div className="mt-4 p-4 bg-gray-800 rounded-lg border border-gray-700">
        <p className="text-gray-400 text-sm">
          <span className="text-green-400 font-bold">How to play:</span> Describe your move in the chat.
          Examples: "Modify the script to add error handling and re-execute" or provide exact Ruby code.
          Commands: <code className="text-yellow-400">reset</code>, <code className="text-yellow-400">state</code>, <code className="text-yellow-400">help</code>
        </p>
      </div>
    </div>
  );
